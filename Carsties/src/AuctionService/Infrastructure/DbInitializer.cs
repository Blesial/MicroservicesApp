...
{
	// We need to create a scope to inject the db Context service.

	// Why? A scoped service is created once per scope. In web applications, this scope is usually tied to an HTTP request.
	//  However, when running code outside of a request 
	// (like during application startup for seeding data), you need to manually create a scope.
	public static void InitDb(WebApplication application)
	{
		// Using keyword allows us to dispose any of the services we used inside this scope
		using var scope = application.Services.CreateScope();
		// Get Service -> This method tells the dependency injection container to provide an instance of the requested service (resolving)
		SeedData(scope.ServiceProvider.GetService<AuctionDbContext>());
		// resolving it within a scope ensures you get a valid instance that's properly managed by the DI container.
	}

	private static void SeedData(AuctionDbContext context)
	{
		context.Database.Migrate(); // Apply any "pending" migation or creates the db if it doesn't exists.
		...
		context.AddRange(auctions); // populates auctions IN MEMORY. 
